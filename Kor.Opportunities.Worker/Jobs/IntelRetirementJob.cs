...
public sealed class IntelRetirementJob : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        var person = await RetireStaleAsync(con, "opportunities.IntelPerson", "not-seen-180-days", StaleDays, ct).ConfigureAwait(false);
        var affiliation = await RetireStaleAsync(con, "opportunities.IntelPersonAffiliation", "not-seen-180-days", StaleDays, ct).ConfigureAwait(false);
        var signal = await RetireStaleAsync(con, "opportunities.IntelSignal", "not-seen-180-days", StaleDays, ct).ConfigureAwait(false);
        var work = await RetireStaleAsync(con, "opportunities.IntelWork", "not-seen-180-days", StaleDays, ct).ConfigureAwait(false);
        var risk = await RetireStaleAsync(con, "opportunities.IntelRisk", "not-seen-180-days", StaleDays, ct).ConfigureAwait(false);
        var narrative = await RetireStaleAsync(con, "opportunities.IntelNarrative", "not-seen-180-days", StaleDays, ct).ConfigureAwait(false);
        var actionStale = await RetireStaleAsync(con, "opportunities.IntelAction", "not-seen-180-days", StaleDays, ct).ConfigureAwait(false);
    ...
    private static async Task<int> RetireStaleAsync(SqlConnection con, string table, string reason, int days, System.Threading.CancellationToken ct)
    {
        var sql = $@"
UPDATE {table}
 ...
 WHERE RetiredAtUtc IS NULL
   AND LastSeenAtUtc < DATEADD(DAY, -@days, sysdatetimeoffset());";
