...
<#
.SYNOPSIS
    Publishes Kor.Operations.FileSync.Service to a timestamped folder.
...
.EXAMPLE
    .\publish.ps1
    # Output: C:\VIsual Studio Projects\_Publish\_Ops\FileSync\20260501_143022\
#>
[CmdletBinding()]
param(
    [string]$PublishRoot = 'C:\VIsual Studio Projects\_Publish\_Ops\FileSync',
    [int]$KeepLast = 3
)

$ErrorActionPreference = 'Stop'
$repoRoot = $PSScriptRoot
$projectPath = Join-Path $repoRoot 'Kor.Operations.FileSync.Service\Kor.Operations.FileSync.Service.csproj'
...
foreach ($script in @('install-service.ps1', 'uninstall-service.ps1')) {
...
Write-Host ""
Write-Host "Next: .\deploy.ps1 -Source `"$out`""
