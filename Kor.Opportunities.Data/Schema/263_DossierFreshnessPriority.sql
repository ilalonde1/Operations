...
/* =====================================================================
   263 — Freshness in the Dossier Completeness priority engine.
   ---------------------------------------------------------------------
   vDossierCompleteness ranked enrichment by Importance x MissingFraction,
   but MissingFraction only counted MISSING flags — a fully-flagged dossier
   scored 0 and never re-enriched even when its intel went stale. That makes
   the system prioritize, but not RE-prioritize.

   This adds a 7th dossier dimension: Fresh (a FirmNarrative/Honing enrichment
   updated within 90 days). A complete-but-aging dossier loses the Fresh flag,
   its MissingFraction rises, its Score climbs back up, and the (already
   Score-ordered) Generate-Batch picks it up for a refresh — no generator
   change needed. The whole pipeline becomes freshness-aware automatically.

   Sibling views (vDossierCompletenessPeople / vDossierCompletenessPursuits)
   are unchanged.
   ===================================================================== */
