...
-- Municipal entity dedup — only obvious dupes where same place has two
-- canonical rows (different prefixes). Excluded:
--   - Langley: "City of Langley" + "Township of Langley" are TWO
--     different municipalities (City of Langley is a separate small city
--     surrounded by Township of Langley).
--   - North Vancouver: "City of North Vancouver" + "District of North
--     Vancouver" are TWO different municipalities.

BEGIN TRAN;

DECLARE @pairs TABLE (LoserId BIGINT PRIMARY KEY, SurvivorId BIGINT NOT NULL);
INSERT INTO @pairs VALUES
    -- Beaumont became a City in 2019; Town -> City
    (53271, 72332),
    -- Chestermere is a City; Town -> City
    (53309, 70302),
    -- Mission is legally a District; City -> District
    (441, 18859),
    -- Pitt Meadows is a City; District -> City
    (71075, 856),
    -- North Cowichan is officially a District; Municipality -> District
    (69988, 18860),
    -- Spruce Grove is a City; Town -> City
    (70294, 304);

-- ========== Aliases ==========
INSERT INTO opportunities.OrgAlias (CanonicalOrgId, RawName, Source, CreatedAtUtc)
SELECT p.SurvivorId, co.DisplayName, N'R95DirectDedup81', sysdatetimeoffset()
FROM @pairs p
...
PRINT 'Migration 81 municipal dedup complete.';
