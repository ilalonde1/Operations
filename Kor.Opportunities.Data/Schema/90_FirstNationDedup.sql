...
-- First Nations canonical dedup. Conservative — keeping:
--   * Buyer-side Nation entities distinct from Developer-side
--     Nation-Development-Corporation entities (they play different
--     roles in BD: the Nation issues RFPs, the Dev Corp delivers
--     projects).
--   * All JV partnership rows (Musqueam + Polygon, MST + Aquilini,
--     Squamish / OPTrust, etc.) — real partnerships.
--   * Different lines of business (Songhees Creek Hydro, Tsawwassen
--     Shuttle's — separate businesses that share the Nation name).
--
-- Only merging name-variant duplicates that are clearly the same
-- legal entity.
...
INSERT INTO @pairs VALUES
    -- Squamish Nation (Buyer) -> 71489
    (53901, 71489),  -- "Squamish First Nation" — same Nation, name variant
    (72216, 71489),  -- "Squamish Nation (100% ownership)" — parenthetical descriptor
    -- Squamish Nation's development arm — Nch'kay Development Corporation -> 72323
    (73797, 72323),  -- "Nch'kay Development Corporation (Squamish Nation)" — same Dev Corp

    -- Musqueam Capital Corporation -> 54985
    (69406, 54985),  -- "Musqueam Capital Corporation (MCC)" — acronym parenthetical

    -- Tsleil-Waututh Nation -> 71268
    (59561, 71268),  -- "TSLEIL WAUTUTH FIRST NATION" — all-caps Vendor variant

    -- Tsawwassen First Nation -> 53989
    (72331, 53989),  -- "Tsawwassen First Nation Economic Development" — descriptor
                     -- Kept separate: 54987 "Tsawwassen Economic Development Corporation"
                     -- (Developer Kind — the Nation's separate dev corp entity)

    -- Tk'emlúps te Secwépemc -> 69369
    (69323, 69369),  -- "Tk'emlúps te Secwépemc First Nation"

    -- Cowichan Tribes -> 630
    (70071, 630);    -- "Cowichan Tribes (Khowutzun Development LLP)" — name+arm descriptor
                     -- Kept separate: 69186 "Khowutzun Development Corporation"
                     -- (the actual Dev Corp entity)

-- ========== Aliases ==========
INSERT INTO opportunities.OrgAlias (CanonicalOrgId, RawName, Source, CreatedAtUtc)
SELECT p.SurvivorId, co.DisplayName, N'R95DirectDedup90', sysdatetimeoffset()
FROM @pairs p
...
PRINT 'Migration 90 First Nations dedup complete.';
