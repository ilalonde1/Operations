...
-- Second BC school-district dedup pass. Migration 75 caught the most
-- obvious variants but my regex missed SD canonicals using "No. N" /
-- slash-suffix / Client kind. Same direct-FK-repoint pattern.

BEGIN TRAN;

DECLARE @pairs TABLE (LoserId BIGINT PRIMARY KEY, SurvivorId BIGINT NOT NULL);
INSERT INTO @pairs VALUES
    (69045, 54415), -- SD23 'School District No. 23 (Central Okanagan / Kelowna)' -> 'School District 23'
    (53678, 54964), -- SD36 'School District 36' -> 'Surrey School District No. 36' (Client kind, more refs)
    (69448, 54965), -- SD37 'Delta School District (SD 37)' -> 'Delta School District No. 37'
    (74047, 54965), -- SD37 'School District 37 (Delta)' -> 'Delta School District No. 37'
    (70260, 54966), -- SD38 'School District No. 38 (Richmond) — Comparator/Template' -> 'Richmond School District No. 38'
    (54967, 53840), -- SD39 'Vancouver School District No. 39' (Client) -> 'School District 39'
    (72173, 53687), -- SD40 'New Westminster School District No. 40' -> 'School District 40'
    (72165, 69301), -- SD42 'Maple Ridge-Pitt Meadows School District No. 42' -> '(SD 42)'
    (54969, 53966), -- SD43 'Coquitlam School District No. 43' (Client) -> 'School District 43'
    (69299, 54970), -- SD44 'North Vancouver School District (SD 44)' -> '...No. 44'
    (74044, 54970), -- SD44 'School District 44 (North Vancouver)' -> '...No. 44'
    (74003, 54972), -- SD61 'School District 61 (Greater Victoria)' -> 'Greater Victoria School District No. 61'
    (74085, 54972), -- SD61 'School District No. 61 (Greater Victoria)' -> 'Greater Victoria School District No. 61'
    (74079, 74004), -- SD62 'Sooke School District No. 62' -> 'School District 62 (Sooke)'
    (74007, 59699), -- SD70 'School District 70 (Pacific Rim)' -> 'SCHOOL DISTRICT NO. 70 ALBERNI'
    (74080, 69991), -- SD71 'School District No. 71 (Comox Valley)' -> '...(Comox Valley) / BGC...'
    (71403, 18900), -- SD73 'School District No. 73 (Kamloops-Thompson)' -> '...Kamloops - Thompson'
    (74092, 18900), -- SD73 'Kamloops-Thompson School District No. 73' -> '...Kamloops - Thompson'
    (74078, 74010), -- SD79 'Cowichan Valley School District No. 79' -> 'School District 79 (Cowichan Valley)'
    (74090, 69177), -- SD83 'School District No. 83 (North Okanagan-Shuswap)' -> 'School District 83 (...)'
    (69052, 53773); -- SD93 'Conseil scolaire francophone (SD 93) / Greater Victoria SD 61' -> '...Francophone'
...
SELECT p.SurvivorId, co.DisplayName, N'R95DirectDedup79', sysdatetimeoffset()
FROM @pairs p
...
-- ========== Step 2: repoint FKs across all tables ==========
...
-- Enrichment collision handling — delete ALL loser enrichments. The
-- survivor will auto-re-enrich on the next research cron. Cleaner than
-- trying to merge two enrichment graphs.
...
UPDATE e SET e.CanonicalOrgId = p.SurvivorId FROM opportunities.CanonicalOrgEnrichment e INNER JOIN @pairs p ON p.LoserId = e.CanonicalOrgId;
PRINT 'Loser enrichments moved: ' + CONVERT(varchar(20), @@ROWCOUNT);
...
PRINT 'Migration 79 second-pass SD dedup complete.';
