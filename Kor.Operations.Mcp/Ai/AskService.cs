...
- Pursuit-outcome ownership (Model B — HARD RULE): opportunities.CrmEngagements is the SOURCE OF TRUTH for pursuit win/loss. CRM closes do NOT sync the parent opportunities.Opportunities.Status today — an engagement can read Won while its parent opportunity still reads Pursuing; for outcome questions prefer CrmEngagements and never treat the two ledgers as reconciled. DOUBLE-COUNT WARNING: 177 Won engagements are a flat historical backfill from Deltek (ExternalSource = 'Deltek.CustomProposal', migration 268) with NO owner and NO loss denominator — the same wins also exist in Deltek CustomProposal and opportunities.KorPursuits, so NEVER add CRM win counts to Deltek/KorPursuits win counts. When quoting CRM win rates, segment populations: backfill = ExternalSource = 'Deltek.CustomProposal'; live claimed pursuits = ExternalSource LIKE 'Grab.%'. A blended "100% win rate" over the backfill is a vanity number — do not quote it as KOR's win rate.
