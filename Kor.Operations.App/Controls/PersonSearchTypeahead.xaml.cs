...
public partial class PersonSearchTypeahead : UserControl
{
    private readonly DispatcherTimer _debounceTimer;
    private CancellationTokenSource? _searchCts;
    private bool _suppressTextChanged;

    public PersonSearchTypeahead()
    {
        InitializeComponent();
    ...
    public event EventHandler<long>? PersonSelected;

    public Func<string, CancellationToken, Task<IReadOnlyList<PersonSearchRow>>>? Search { get; set; }
    ...
    private void SearchBox_KeyDown(object sender, KeyEventArgs e)
    {
        ...
        {
            if (ResultsList.SelectedItem is not PersonSearchRow)
            {
    ...
    {
        if (ResultsList.SelectedItem is not PersonSearchRow row)
        {
            return;
        }
        ...
        SearchBox.CaretIndex = SearchBox.Text.Length;
        ResultsPopup.IsOpen = false;
        PersonSelected?.Invoke(this, row.IntelPersonId);
    }
