    ...
    internal static class PdfGeometryExtractor
    {
            ...
            string? loadCombCode = null)
        {
            ...
            for (int storyIndex = 0; storyIndex < stories.Count; storyIndex++)
            {
                ...
                for (int i = 0; i < xSlabs.Count; i++)
                {
                    var pts = xSlabs[i];
                    var color = xSlabColors[i];
                    areas.Add(($"s{storyIndex + 1}_A{++areaCounter}", pts.Select(p => Pt(p.X, p.Y)).ToList(), storyName, PropNameFor(color), color));
                }
            ...
            using var sw = new StreamWriter(outputPath, false, System.Text.Encoding.ASCII);
            sw.WriteLine("$ ETABS v9 TEXT FILE");
            sw.WriteLine("$   Generated by KOR NewerForma - PDF to SAFE/ETABS");
            sw.WriteLine();
            ...
            Dictionary<(byte R, byte G, byte B), SlabColorSettings>? colorSettings = null)
        {
            ...
            List<(double X, double Y)> FilterPts(List<(double X, double Y)> raw)
            {
                var result = new List<(double X, double Y)>();
                foreach (var p in raw)
                    if (Ok(p.X, p.Y) && (result.Count == 0 || Distance(result[^1], p) >= 0.5))
                        result.Add(p);
                return result;
            }
            static double TriArea((double X, double Y) a, (double X, double Y) b, (double X, double Y) c)
            ...
            for (int storyIndex = 0; storyIndex < orderedStories.Count; storyIndex++)
            {
                ...
                for (int i = xSlabs.Count - 1; i >= 0; i--)
                {
                    var simplified = DouglasPeucker(xSlabs[i], 5.0);
                    if (simplified.Count >= 3) xSlabs[i] = simplified;
                    else { xSlabs.RemoveAt(i); xSlabColors.RemoveAt(i); }
                }
                ...
                for (int i = 0; i < xSlabs.Count; i++)
                {
                    if (xSlabs[i].Count <= 4)
                    {
                        decomposed.Add(xSlabs[i]);
                        decomposedColors.Add(xSlabColors[i]);
                        continue;
                    }
                    for (int j = 1; j < xSlabs[i].Count - 1; j++)
                    {
                        if (TriArea(xSlabs[i][0], xSlabs[i][j], xSlabs[i][j + 1]) > 100.0)
                        {
                            decomposed.Add(new List<(double X, double Y)> { xSlabs[i][0], xSlabs[i][j], xSlabs[i][j + 1] });
                            decomposedColors.Add(xSlabColors[i]);
                        }
                    }
                }
            ...
            var gradeProps = new Dictionary<string, (double E, double G, double Fc)>
            {
                { "C20", (29962, 12804, 20) }, { "C25", (31476, 13451, 25) },
                { "C28", (32308, 13806, 28) }, { "C30", (32837, 14033, 30) },
                { "C32", (33346, 14251, 32) }, { "C35", (34077, 14563, 35) },
                { "C40", (35220, 15051, 40) }, { "C50", (37278, 15930, 50) },
            };
            var usedGrades = areas.Select(a => GradeFor(a.Color)).Distinct().OrderBy(g => g).ToList();
            if (usedGrades.Count == 0) usedGrades.Add("C30");
            ...
            {
                var (e, _, fc) = gradeProps.TryGetValue(grade, out var p) ? p : (32837, 14033, 30);
            ...
            var uniqueSlabProps = areas
                .Select(a =>
                {
                    var settings = colorSettings != null && colorSettings.TryGetValue(a.Color, out var s)
                        ? s : new SlabColorSettings();
                    return (a.PropName, ThicknessM: (settings.ThicknessMm > 0 ? settings.ThicknessMm : 200.0) / 1000.0,
                        Grade: !string.IsNullOrWhiteSpace(settings.GradeCode) ? settings.GradeCode : "C30");
                })
                .Distinct()
                .OrderBy(p => p.PropName)
                .ToList();
