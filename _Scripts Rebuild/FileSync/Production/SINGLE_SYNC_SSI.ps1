...
$LogFile = "C:\_APPS\FileSync\Production\Logs\Single_SSI_Sync_Log.txt"
...
function Upload-SharePointFiles {
    ...
    # Get existing SharePoint files (once)
...
# Extract project folder (GivenPath is ...\<Project>\04 Construction Admin\02 SSI (Structural Site Instructions))
$ProjectDir  = Split-Path (Split-Path $GivenPath -Parent) -Parent
$ProjectName = Split-Path $ProjectDir -Leaf

# Target in SharePoint: <Project>/SSI  (no "04 Construction Admin")
$SPRoot   = $ProjectName
$SPFolder = "$SPRoot/SSI"
