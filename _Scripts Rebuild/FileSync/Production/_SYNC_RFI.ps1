# Rename script to _SYNC_RFI.ps1
...
$TargetFolderPath = "04 Construction Admin\03 RFI (Request for Info)\Sent to Inspectors"
$SPBaseFolder = ""
$LogFile = "C:\_APPS\FileSync\Production\Logs\RFI_Sync_Log.txt"
...
foreach ($Category in $ProjectFolders) {
    ...
    foreach ($Project in $Projects) {
        ...
        # Define paths
        $LocalRFIPath = Join-Path -Path $Project.FullName -ChildPath $TargetFolderPath
        $SPProjectFolder = "$SPBaseFolder/$($Project.Name)"
        $SPRFIFolder = "$SPProjectFolder/RFI"

        # Check if local folder exists
        $LocalFiles = @()
        if (Test-Path $LocalRFIPath) {
            # Get only PDF files in one efficient call
            $LocalFiles = Get-ChildItem -Path $LocalRFIPath -File -Filter "*.pdf"
        } else {
            Log-Message "WARNING: Local RFI folder does not exist for project: $($Project.Name). Proceeding to delete SharePoint files if necessary."
        }

        # Check if SharePoint folders exist
        if (-not $global:CachedSPFolders.ContainsKey($SPRFIFolder)) {
    $global:CachedSPFolders[$SPRFIFolder] = Test-SharePointFolder -FolderPath $SPRFIFolder
}
$RFIFolderExists = $global:CachedSPFolders[$SPRFIFolder]

        # Retrieve SharePoint files (always do this, even if there are no local files)
        $SPFiles = @{}  
        if ($RFIFolderExists) {
            foreach ($File in (Get-SharePointFiles -FolderPath $SPRFIFolder)) {
...
if (-not $ProjectFolderExists) {
    ...
    $global:CachedSPFolders[$SPProjectFolder] = $true  # Update cache
            ...
            if (-not $RFIFolderExists) {
    New-SharePointFolder -ParentPath $SPProjectFolder -FolderName "RFI"
    $RFIFolderExists = $true
}
...
foreach ($File in $LocalFiles) {
    if (-not $SPFiles.ContainsKey($File.Name)) {
        Log-Message "File not found in SharePoint, will upload: $($File.Name)"
        $FilesToUpload += $File
    } elseif ($File.Length -ne $SPFiles[$File.Name]) {
        Log-Message "File size mismatch, will upload: $($File.Name) (Local: $($File.Length), SP: $($SPFiles[$File.Name]))"
        $FilesToUpload += $File
    } else {
...
if ($FilesToUpload.Count -gt 0) {
    Upload-SharePointFiles -FilesToUpload $FilesToUpload -FolderPath $SPRFIFolder
} else {
    Log-Message "No new or updated files to upload for $SPRFIFolder"
}
