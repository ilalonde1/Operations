...
$SPBaseFolder = ""
$LogFile = "C:\_APPS\FileSync\Production\Logs\SSI_Sync_Log.txt"
...
foreach ($Category in $ProjectFolders) {
    ...
    foreach ($Project in $Projects) {
        ...
        $SPSSIFolder = "$SPProjectFolder/SSI"

        # Check if local folder exists
        $LocalFiles = @()
        if (Test-Path $LocalSSIPath) {
            # Get only PDF files in one efficient call
            $LocalFiles = Get-ChildItem -Path $LocalSSIPath -File -Filter "*.pdf"
        } else {
            Log-Message "WARNING: Local SSI folder does not exist for project: $($Project.Name). Proceeding to delete SharePoint files if necessary."
        }

        # Check if SharePoint folders exist
        if (-not $global:CachedSPFolders.ContainsKey($SPSSIFolder)) {
    $global:CachedSPFolders[$SPSSIFolder] = Test-SharePointFolder -FolderPath $SPSSIFolder
}
$SSIFolderExists = $global:CachedSPFolders[$SPSSIFolder]

        # Retrieve SharePoint files (always do this, even if there are no local files)
        $SPFiles = @{}  
        if ($SSIFolderExists) {
            foreach ($File in (Get-SharePointFiles -FolderPath $SPSSIFolder)) {
...
if (-not $ProjectFolderExists) {
    ...
    $global:CachedSPFolders[$SPProjectFolder] = $true  # Update cache
            ...
            if (-not $SSIFolderExists) {
                New-SharePointFolder -ParentPath $SPProjectFolder -FolderName "SSI"
                $SSIFolderExists = $true
            }
...
if ($FilesToUpload.Count -gt 0) {
    Upload-SharePointFiles -FilesToUpload $FilesToUpload -FolderPath $SPSSIFolder
} else {
    Log-Message "No new or updated files to upload for $SPSSIFolder"
}
