...
function Try-Run([string]$root, [string]$scriptPath, [switch]$Force, [switch]$IgnoreControl) {
  if ([string]::IsNullOrWhiteSpace($root)) { Log "ABORT: empty root passed to $scriptPath"; return }
  if (-not (Test-Path -LiteralPath $root)) { Log "ABORT: root does not exist for $scriptPath : $root"; return }
  if (-not (Test-Path -LiteralPath $scriptPath)) { Log "ABORT: script not found: $scriptPath"; return }
  ...
  try {
    ...
    & $scriptPath -GivenPath $root   # <-- FIXED: invoke the path variable, not $script:scriptPath
  } catch {
