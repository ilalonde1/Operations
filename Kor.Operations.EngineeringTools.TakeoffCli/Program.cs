...
public static class TakeoffCliHelp
{
    ...
    [
        ...
        new("corpus-query", "takeoff corpus-query summary|no-model|plan-titles|set <job>...|plates|yardsticks|diff <before-sets.csv>|pages <job> [--runs a,b] [--ledger <dir>]", "Questions to the corpus ledger, no PDF opened: the population in one table, every set without a model by its reason, how plan sheets name their storeys, one set's sheets, the yardsticks worst first."),
