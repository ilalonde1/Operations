...
public static class TakeoffCliHelp
{
    ...
    [
        ...
        new("pdf-at", "takeoff pdf-at <pdf> <page> <x> <y> --scale N [--radius mm] [--rules-db <conn>] [--points]", "What happened to the ink here: every path and word within a radius of a point (page mm, the frame of pdf-overlay --mark), each with the fate the intake gave it."),
        ...
        new("dxf-strip", "takeoff dxf-strip <dxfFolder> <outFolder> --layers SLABEDG[,BEAM] [--sheets \"S2.02*.dxf\"]", "The strip-a-layer differential, first half: copy a set's DXFs with one layer's entities moved to X-STRIPPED; then dxf-to-etabs both folders and model-diff the models. Three minutes that settle whether a move is the composer's or the layer's."),
        new("dxf-inspect", "takeoff dxf-inspect <plan.dxf> [--walls] [--plates] [--loops] [--faces [--layers A,B]] [--near <x> <y> [reach]]", "Inspect DXF layers, loops, wall outlines, recovered floor plates; --loops lists loops whose centroid is the vertex mean; --faces the planar faces of the lines and the open chains that keep an edge from closing; --near every segment on any layer within a reach of a point, with its layer."),
        ...
        new("vector-lines", "takeoff vector-lines <pdf> <page> [--region x0 y0 x1 y1] [--min-pt 20] [--scale 96] [--pens] [--long]", "GROUND TRUTH: every long axis-aligned run the page itself draws, grouped by the line it sits on, in points and drawing mm - is the drafter's line there, before a reader is blamed."),
        ...
        new("model-render", "takeoff model-render <model.e2k> <out.png|out.svg> [\"title\"] [--columns N] [--cell px] [--no-png] [--storey L2[,L3]]", "Every storey of a model on one sheet - walls dark red, columns green, each plate its own colour - so a person can LOOK before counting. SVG always, PNG through Edge."),
        ...
        new("model-at", "takeoff model-at <model.e2k> <x> <y> [reach]", "What the model holds within a reach of a plan point, nearest first: kind, ends and the storeys it rises through — the model half of the question dxf-inspect --near asks of the drawing."),
        new("corpus-analyze", "takeoff corpus-analyze [--census] [<projectsRoot>] [--work <dir>] [--jobs a,b] [--parallel N] [--force] [--rules-db <conn>]", "The whole corpus through the one ingestion point: every job's current stick file mirrored once and built as the verbs build one, one row per set and per sheet into analysis.IntakeSet / IntakeSheet (migration 083) and CSV beside the work; then X of Y build, and why the rest do not."),
        ...
        new("corpus-gate", "takeoff corpus-gate [--baseline <ledger-sets.csv>] [--jobs a,b] [--parallel N] [--out <csv>] [--no-build] [--recompose] [--bisect KOR_STEP<n>_OFF,...]", "THE GATE BETWEEN A RULE AND ITS BANK: builds the sets the engineer has a model of and judges them against a banked ledger on HER figures - plate area, slab thickness, openings - naming every set that LOST. The six-set gate says a rule broke nothing; this says whether it is good. --recompose keeps the last run's views and runs only the composer again, which is right for a composer-side rule and wrong for a reader-side one. Exit 1 on a loss."),
        new("corpus-query", "takeoff corpus-query summary|no-model|plan-titles|set <job>...|plates|yardsticks|frames|dropped|diff <before-sets.csv>|pages <job> [--runs a,b]|storeys|grid-names [--ledger <dir>]", "Questions to the corpus ledger, no PDF opened: the population in one table, every set without a model by its reason, how plan sheets name their storeys, one set's sheets, the yardsticks worst first, what stacks by its page frame, and every plan sheet that read slabs and gave the model no storey."),
