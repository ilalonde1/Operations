...
//   takeoff vector-lines <pdf> <page> [--region x0 y0 x1 y1] [--min-pt 20] [--scale 96]
internal static class VectorLinesVerb
{
    ...
    {
        if (args.Length < 3) { Console.Error.WriteLine("Usage: takeoff vector-lines <pdf> <page> [--region x0 y0 x1 y1] [--min-pt 20] [--scale 96]"); return 1; }
        ...
        foreach (var path in pc.Paths)
        {
            ...
            for (int k = 0; k < edges; k++)
            {
                ...
                n++;
                if (Math.Abs(a.Y - b.Y) < 0.3 && Math.Abs(a.X - b.X) >= minPt) Add(hor, Math.Round(a.Y, 1), Math.Min(a.X, b.X), Math.Max(a.X, b.X));
                if (Math.Abs(a.X - b.X) < 0.3 && Math.Abs(a.Y - b.Y) >= minPt) Add(ver, Math.Round(a.X, 1), Math.Min(a.Y, b.Y), Math.Max(a.Y, b.Y));
            }
        }
