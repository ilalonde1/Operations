...
Add-Check $S 'App.config Approvers|Publishers' 'ilalonde@korstructural.com;sacosti@korstructural.com;rbeirne@korstructural.com|ilalonde@korstructural.com' { $c = Get-Content (Join-Path $RepoRoot 'Kor.Operations.App\App.config') -Raw; [regex]::Match($c, 'StandardDetailsApprovers\.Members"\s+value="([^"]+)"').Groups[1].Value + '|' + [regex]::Match($c, 'StandardDetailsPublishers\.Members"\s+value="([^"]+)"').Groups[1].Value }
