...
"@
    $cmd.Parameters.Add('@pattern', [System.Data.SqlDbType]::NVarChar, 256).Value = "%$archName%"
    $r = $cmd.ExecuteReader()
    $projs = @()
    while ($r.Read()) {
...
foreach ($a in $archs[0..9]) {
    ...
    $projs = GetArchProjects $a.Architect
    if ($projs.Count -gt 0) {
...
P "$($korAligned.Count) architects in top 40 are already in KOR's memory client list. These are immediate warm-intro candidates — KOR likely has a Principal-in-Charge relationship to leverage."
...
P "$($newTargets.Count) architects in top 40 do NOT appear in KOR's memory client list. These represent NEW relationship-building targets. Combined Total Refs across these architects = $((($newTargets | Measure-Object TotalRefs -Sum).Sum)) Intel signals/actions — compounding pursuit potential."
