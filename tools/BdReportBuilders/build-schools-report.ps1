...
try {
    ...
    H2 ("3. DEAD — Awarded or unviable ($($dead.Count) projects)")

    P ("Significant DB hygiene observation: $($dead.Count) of $($briefs.Count) honed school MPIs are DEAD. This includes structural engineers already awarded, projects delivered, or speculative MPIs without procurement approval. The DEAD list is below for reference only — these are NOT active pursuit targets.")
    ...
    H2 ("4. DISCOVER — Pre-procurement SD relationship-build ($($discover.Count) projects)")
