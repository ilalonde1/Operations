...
# BD-Audit-2026-06-09 M12: Cost is a mixed-format string ('~$2 billion (CAD)',
# '1959000000', '1500', '$45M') — Sort-Object on the raw string sorts lexically
# ('810000000' ranked above '1959000000'). CostNumeric parses to a double for
# ordering only; the original string is what gets displayed.
function CostNumeric { param($v)
    if ($null -eq $v) { return 0.0 }
    $s = ([string]$v).Trim()
    if (-not $s) { return 0.0 }
    $mult = 1.0
    if ($s -match '(?i)billion' -or $s -match '(?i)\d\s*B\b') { $mult = 1e9 }
    elseif ($s -match '(?i)million' -or $s -match '(?i)\d\s*M\b') { $mult = 1e6 }
    $m = [regex]::Match(($s -replace ','), '\d+(\.\d+)?')
    if (-not $m.Success) { return 0.0 }
    return ([double]$m.Value) * $mult
}
...
try {
    ...
    if ($korAligned.Count -gt 0) {
        ...
        P ''
        $rows = @()
        foreach ($p in ($korAligned | Sort-Object -Property { CostNumeric $_.Cost } -Descending)) {
            $rows += ,@(
                "$($p.Id)",
                (Safe $p.Name 50),
                (Safe $p.PrimeFirm 40),
    ...
    $top30 = $withPrime | Sort-Object -Property { CostNumeric $_.Cost } -Descending | Select-Object -First 30
    $rows = @()
    foreach ($p in $top30) {
        $rows += ,@(
            "$($p.Id)",
    ...
    $rank = 1
    foreach ($p in ($withPrime | Sort-Object -Property { CostNumeric $_.Cost } -Descending | Select-Object -First 10)) {
        H3 ("$rank. $($p.Name)")
        B 'Project Id: ' "$($p.Id)"
    ...
    if ($withoutPrime.Count -gt 0) {
        ...
        P ''
        $top20unknown = $withoutPrime | Sort-Object -Property { CostNumeric $_.Cost } -Descending | Select-Object -First 20
        $rows = @()
