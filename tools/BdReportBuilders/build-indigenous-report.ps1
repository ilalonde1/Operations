...
# Ids 7161-7166 are defense/military MPIs (CFHA housing + DRDC + STTCP
# discoveries) that the indigenous pull query also matched; they are covered
# by build-defense-report.ps1. Maintain this exclusion list explicitly —
# do not re-introduce range-based magic-Id filters (BD-Audit-2026-06-09 M12).
$excludedDefenseOverlapIds = @(7161, 7162, 7163, 7164, 7165, 7166)
$indigenous = $briefs | Where-Object { $excludedDefenseOverlapIds -notcontains $_.Id }
...
try {
    ...
    H2 ("1. PURSUE — Open opportunities ($($pursueOnly.Count) projects)")

    P 'Each project flagged as PURSUE has either an active procurement window OR a specific named warm-intro path identified by Sonnet. Organized by Nation/sponsor type; anything not matching a named grouping renders under "Other PURSUE" so no brief is dropped.'
    ...
    # BD-Audit-2026-06-09 M12: the five name-regex buckets above silently dropped
    # any PURSUE brief that matched none of them (verified: Id 4877 'Langara
    # Family YMCA' missed the 'Langara YMCA' pattern). Catch-all renders the rest.
    H3 '1.6 Other PURSUE projects'
    $bucketedIds = @($squamish + $lowerMainland + $island + $interior + $ab | ForEach-Object { $_.Id })
    $otherPursue = @($pursueOnly | Where-Object { $bucketedIds -notcontains $_.Id })
    if ($otherPursue.Count -gt 0) {
        foreach ($p in $otherPursue) {
            B ("Id $($p.Id): ") "$($p.Name) ($($p.Province))"
            P (Safe $p.korAngle 500)
            Italic ("Status: " + $p.status)
            P ''
        }
    } else { P 'None — every PURSUE brief matched a named grouping above.' }

    H2 ("2. MONITOR — Relationship-building targets ($($monitorOnly.Count) projects)")
    ...
    H2 ("3. DEAD — Awarded or unviable ($($deadOnly.Count) projects)")
    ...
    H2 ("4. DISCOVER — Pre-procurement watch ($($discoverOnly.Count) projects)")
