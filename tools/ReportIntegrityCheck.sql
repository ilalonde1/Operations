...
;WITH lm AS (
  SELECT m.*, LOWER(ISNULL(NULLIF(LTRIM(RTRIM(m.ProjectStage)),''),'')) St,
    ...
    CASE WHEN ISNULL(m.Sector,'') LIKE '%Utilit%' OR ISNULL(m.Sector,'') LIKE '%Infrastructure%'
           ...
           OR m.ProjectName LIKE '%Terminal%' OR m.ProjectName LIKE '%Substation%' OR m.ProjectName LIKE '%Pipeline%' THEN 1 ELSE 0 END OutOfLane
...
early AS (SELECT * FROM lm WHERE NoSE=1 AND St<>'' AND St NOT LIKE '%construction%' AND St NOT LIKE '%complete%' AND St NOT LIKE '%on hold%')
SELECT Chk, CAST(Val AS NVARCHAR(20)) Value, Result FROM (
  ...
  UNION ALL SELECT 2,'INFO: early-open (no SE on file)', (SELECT COUNT(*) FROM early),'INFO'
  ...
  UNION ALL SELECT 4,'GATE: 0 Completed/built shown active', (SELECT COUNT(*) FROM lm WHERE St='completed'),
    CASE WHEN (SELECT COUNT(*) FROM lm WHERE St='completed')=0 THEN 'PASS' ELSE 'FAIL' END
  UNION ALL SELECT 5,'GATE: 0 construction-stage in early-open', (SELECT COUNT(*) FROM early WHERE St LIKE '%construction%'),
    CASE WHEN (SELECT COUNT(*) FROM early WHERE St LIKE '%construction%')=0 THEN 'PASS' ELSE 'FAIL' END
  UNION ALL SELECT 6,'GATE: 0 on-hold in early-open', (SELECT COUNT(*) FROM early WHERE St LIKE '%on hold%'),
    CASE WHEN (SELECT COUNT(*) FROM early WHERE St LIKE '%on hold%')=0 THEN 'PASS' ELSE 'FAIL' END
) x ORDER BY ord;
GO
