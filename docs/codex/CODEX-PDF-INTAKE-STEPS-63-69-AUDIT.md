# Codex — adversarial audit of intake steps 63–71 and the night's infrastructure (read cache, page record, BridgeChains, partial ledger, column trace)

**Reading set: about 165 KB at HEAD on `develop` (the commit named in the paste-ready prompt), measured
with `wc -c` before handover — 43 KB of `docs/PdfIntake.md`, 6 KB of `CLAUDE.md`, ~110 KB of source
...
(each claim is in `docs/PdfIntake.md` §72–§81 with WHAT THIS COVERS / WHAT IT DOES NOT):
...
- **Step 71 (§81)** `PlanSheetNaming.Vocabulary` — a process-wide static each composition set to its
  set's ranked words while twelve sets compose at once — is an `AsyncLocal`; the test is proved by
  breaking it.
...
2. `docs/PdfIntake.md` — §72 through §81, from the line `## 72. Step 63` to the end of the file
   (43 KB). Do not read the rest.
...
5. Tests, whole files:
   ...
   - `Kor.Operations.EngineeringTools.Core.Tests/Intake/TheVocabularyInForceIsTheCallersTests.cs` (3 KB)
...
**K. The flow-local vocabulary (§81).** `PlanSheetNaming.Vocabulary` is an `AsyncLocal` read through
`Parse(string)` and four other members. Name every place that sets it (`DxfToEtabsService.Run`,
`PdfOnlyBuild`, `RestoreVocabulary.Dispose`, tests) and say for each whether the reader of that
value is on the same execution flow — in particular `PdfOnlyBuild.WriteSheets` when
`DrawingIntake.ReadPage` is called from a `Parallel.For` body, and `CorpusAnalyzer`'s per-set body,
which sets the office vocabulary and then builds. A value set inside a `Parallel.For` iteration
does not reach the iterations beside it or the caller — is anything relying on it reaching either?

**L. The checks versus their names.** For each test file in the reading set, compare WHAT THIS
...
- The partial ledger holds 279 of 296 rows on run 15: the 17 "stick file of another job" sets are
  decided before the per-set loop and never append (§81). Known; question H asks about the rest.
...
`docs/codex/CODEX-PDF-INTAKE-STEPS-63-69-AUDIT-RESPONSE.md` (the file keeps its name; it covers 63–71). One finding per heading, most severe
