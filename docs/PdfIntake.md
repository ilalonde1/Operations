...
## 0. START HERE (state as of 2026-09-17 04:10, after steps 47 and 54–113 — the evening's openings run: 105b/c, 106, 107/107b, 110 (a jog of the linework is kept), 105d (the landing is the well's), 111 (a sleeve is a box with its diagonals), 113/113b (a walk floor holding few columns is not the page's floor; two plates covering each other are one floor), §115–§126 — and WP7's first block (the EGBC brain: migration 096, `takeoff knowledge-ingest`, 26 sources / 959 clause rows, §116) — 260 of 296 sets build from the PDF alone (run 33, `069690e4`, 04:07); plates on 74% of storeys (2,105 of 2,835); on the corpus her openings we have 25% → 33%, ours she has 38% (70% / 41% over the 22 sets whose model is within a year of the drawing — `corpus-disagreements` section 6 splits by her model's age); on the five gate sets her openings we have 44% → 56%; 31130 +69,869 sq ft of plate. PARKED on branches: 108 (a pour strip; migration 098 written and probed, awaits Ian), 109 (30838's two-view page; behind 112), 112 (a T drawn short: closes 30838's plans, refused by `RecoverSurfaces` on 31065's P1 — Codex brief `docs/codex/CODEX-PDF-INTAKE-STEP-112-A-T-DRAWN-SHORT-REFUSES-A-PAGE.md`, Ian runs it). Three detached runs died on 09-16 (a console close each; `ProcessExit` logged now). The engineers' review comes from the corpus — `takeoff corpus-disagreements` (§111, §114); the engineer's lines in `docs/pdf-intake/QUESTIONS.md`)
