...
## THE JOB IN FRONT OF YOU (2026-09-10, after intake step 40)
...
`docs/PdfIntake.md` **§0 START HERE**, then its §30, then its last three step sections (§47, §48,
§49). That is about seventy lines of state plus three rules. Sections 1 to 29 of that document are
...
the banked `.e2k` baselines named for the step that produced them (`pdf-only-<job>-s40.e2k` is
...
**One command measures every deliverable.**
      ...
      bash docs/etabs-handoff/six_set_diff.sh s40                  # THEN THIS: one line per set against the banked step - byte-identical, or what moved
...
byte-identical. That is the shape to look for. And the run REFUSES rules: step 35's first cut
removed "members outside every plate" — right on 31170, and it took four KOR sets' walls (§44),
because their plates are still fragments. A rule that acts on "outside the plate" counts and
lists until the plate reader is whole.
