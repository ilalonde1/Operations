...
- **1c. Split / clean the name-integrity messes** — ⏸ **CHARACTERIZED 2026-06-21, awaiting review (judgment-gated).** Broad detection = **641 active orgs** with name-integrity signals, but **only ~10 are in active MPI use** (the rest don't corrupt the project graph). Splits into THREE distinct problems:
  ...
  - **(ii) Health-authority concatenations (#794, #695, #73830):** multi-select buyer field run together with NO delimiter (`Fraser Health AuthorityInterior Health Authority…`). The 7 BC HAs already exist as canonicals (PHSA #54962, Fraser Health #76424…). NOT in MPI use. Clean, well-defined split + repoint of procurement refs.
