...
- **Apollo — WORKS on the paid plan (correction, 2026-06-23):** my first-pass "redacted dead-end" call was WRONG. The `api_search` endpoint obfuscates last names by design (discovery preview); the `people/match` endpoint with `reveal_personal_emails` returns the full name + **verified** work email + LinkedIn (enrichment credits). Built `tools/BdApolloEnrich` (commit `40abfb54`) on the proven Schema/162 contract — discover thin orgs → reveal → org-domain guard → dedup-safe persist. **Verified run:** 6 orgs → 17 contacts revealed → 16 linked (e.g. Intracorp President&CEO Don Forsgren, Bucci Director of Construction Don Poirier, Appia VP Ops Francis Petrunia), all verified emails; enrichment parents refreshed so those briefs now show current. Ready to scale — capped by `--max-orgs`/`--per-org` because each reveal spends a credit (your call on how wide to run it).
