...
| 7 | **Storeys named as the set names them** — DONE for the named shapes 2026-09-16 (step 84, §92): LEVEL (L35) → L35; LEVEL -3 → P3 in the ladder AND the plan titles (30912's five parkade plans placed); `corpus-query storeys` classes every name (roofs by word, sub-levels, letter-and-count, elevations stay as the set says them). Residue: 6 garbage names on 5 sets = item 5's class; NEXT ROW (found by step 87, §97): a B-level is a below-grade level — 30941 titles its plans LEVEL B4 and its ladder names the storeys B1–B4, and the vocabulary's ParkadeWords is P alone, so no plan of it places (12 of 138): a KorStandards row (ParkadeWords += B) with the ladder's B-names joined, one step | `corpus-query storeys`: garbage 6 → 0 (item 5) | done |
