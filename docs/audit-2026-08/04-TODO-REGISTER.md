...
| 57 | Give `takeoff.exe` a help/usage list of its real commands | 09 | S | BEFORE-DEMO | Bare invocation prints the usage line for the oldest CSV-diff command, out of ~35 subcommands in a 2,971-line file `[RUN]` | either | briefed 7 |
...
| 112 | Annotate the 2 `async void` methods and comment the 3 cleanup catches | 08 | S | Five minutes; turns the module's test signal green | either | briefed 7 |
...
| 115 | Add a global `DispatcherUnhandledException` handler | 08 | S | There is currently **none anywhere in the app**; five `async void` handlers are safe only by coincidence `[RUN]` | either | briefed 7 |
