...
| 10 | Register `KorMapSync` with Quartz in `QuartzInstaller.cs` (mirror the existing 5-job pattern) | 03 | S | BEFORE-DEMO | Four lines. The UI shows a countdown to a fire that has never happened; the public map is 8 days stale `[QUERIED]` | either | briefed 3 |
| 11 | Fix `FileSyncLogTailer` to use `fs.Length` instead of `FileInfo.Length` | 03 | S | BEFORE-DEMO | "Show me the logs" returns a blank grid on a healthy service. Measured: `FileInfo.Length` 0 vs `Stream.Length` 43,165, same file, same instant `[RUN]` | either | briefed 3 |
| 12 | Honour `KOR_FILESYNC_MODE`, or relabel the heartbeat `GlobalMode` column | 03 | S | BEFORE-DEMO | The panel states the opposite of the truth: `Shadow` while all 7 jobs are `Live` and moving client files `[QUERIED]` | either | briefed 3 |
