...
public sealed class EveryReaderConstantIsTriagedTests
{
    ...
    private static readonly IReadOnlyDictionary<string, Triage> Table = new Dictionary<string, Triage>(StringComparer.Ordinal)
    {
        ...
        ["CorpusGate.cs:OverReadShare"] = Rule("half again her own plate area on a storey is an over-read: a rule that only ADDS cannot lose, so the gate watches what it adds (step 136 and Codex's courtyard void, 2026-09-22)"),
        ...
        ["SheetFurniture.cs:AxisPenHeavierBy"] = Tolerance("a stroke on a grid axis heavier than this many times the grid's pen at its bubble is not the grid: 1.4, since the next pen up is twice (31009's slab edge on its grid, step 128)"),
        ["SheetFurniture.cs:AxisPenLighterBy"] = Tolerance("a stroke on a grid axis lighter than this many times the grid's pen is not the grid; twice, since a 3 pt grid has 2 pt pieces (31202 p32, step 53)"),
