    ...
    public static class SheetInventory
    {
        ...
        public static SheetLedger Of(SheetRecord record)
        {
            ...
            if (classified)
            {
                ...
                if (record.Geometry.Walls.Count > record.Geometry.FirstFaceWall)
                    Note("emitted: walls read from two face lines a wall's thickness apart (step 20)", record.Geometry.Walls.Count - record.Geometry.FirstFaceWall,
